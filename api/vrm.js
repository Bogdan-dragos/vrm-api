// /api/vrm.js
// Returns: { vrm, year, make, model, variant, variantDerived, dvlaModel, fuelType, colour, description, calls }
...
export default async function handler(req, res) {
  ...
  // ---------------- VDG (nested + flat) with dvlaModel + variantDerived ----------------
  try {
    ...
    if (debug) out.calls.vdgRequest = { url: `${vdgBase}/r2/lookup?packagename=${encodeURIComponent(process.env.VDG_PACKAGE || "VehicleDetails")}&vrm=${encodeURIComponent(vrm)}&apikey=***` };

    const r = await fetch(vdgUrl, { method: "GET", headers: { "Accept": "application/json" } });
    const raw = await r.text(); // capture regardless of status
    if (debug) {
      out.calls.vdg = { status: r.status, ok: r.ok };
      out.calls.vdgBodyPreview = raw.slice(0, 600);
    }
    ...
    if (r.ok) {
      ...
      if (j && j.Results) {
        ...
        const vdgModel  = VI.DvlaModel || MI.Model || MI.Range; // compound name often here
        const vdgVar    = MI.ModelVariant || MI.Series || "";   // clean variant when present
        ...
        // Prefer MI.Range for base model when available; fallback to vdgModel
      ...
      if (!usedNested && data && typeof data === "object") {
        ...
        // Try deriving variant from Model if it’s compound
  ...
  // ---------------- Final description (use variantDerived if variant missing) ----------------
